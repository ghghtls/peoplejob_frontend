...
Widget _app({
  ...
  return Provider<JobService>.value(
    value: jobService,
    child: MaterialApp(home: const JobListPage(), routes: routes ?? const {}),
  );
}
...
void main() {
  group('JobListPage Widget Tests', () {
    ...
    testWidgets('채용공고 목록 페이지 기본 UI 렌더링 테스트', (WidgetTester tester) async {
      ...
      await tester.pumpWidget(_app(jobService: mockJobService));
    ...
    testWidgets('채용공고 카드 탭 테스트', (WidgetTester tester) async {
      // Given
      final jobsData = [
        {'jobopeningNo': 1, 'title': '백엔드 개발자 모집', 'company': '테스트 회사'},
      ];
      when(mockJobService.getAllJobs()).thenAnswer((_) async => jobsData);
      ...
      await tester.pumpWidget(
        _app(
          ...
          routes: {
            '/job-detail':
                (context) => const Scaffold(body: Text('Job Detail Page')),
          },
        ),
      );
      await tester.pumpAndSettle();
    ...
    testWidgets('새로고침 버튼 테스트', (WidgetTester tester) async {
      ...
      // Then: 초기 1회 + 새로고침 1회
      verify(mockJobService.getAllJobs()).called(2);
    });
    ...
    testWidgets('FloatingActionButton 테스트', (WidgetTester tester) async {
      ...
      await tester.pumpWidget(
        _app(
          ...
          routes: {
            '/job-register':
                (context) => const Scaffold(body: Text('Job Register Page')),
          },
        ),
      );
      await tester.pumpAndSettle();
    ...
    testWidgets('마감된 채용공고 표시 테스트', (WidgetTester tester) async {
      ...
      // (스타일 검증은 위젯 구현에 따라 다르므로 텍스트만 검증)
    });
    ...
    testWidgets('RefreshIndicator 테스트', (WidgetTester tester) async {
      ...
      await tester.pump(); // 트리거
      await tester.pump(const Duration(seconds: 1)); // 인디케이터 애니메이션 일부 진행
